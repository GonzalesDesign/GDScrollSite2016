...
(function () { //IIFE:Immediately-Invoked Function Expression
  ...
  function fTwoColumns (rightColumnPercentage) {
    ...
    //fAnimateLeftPosition (imgTitle, -30)
  ...
  function fNextPreviousButtonsXXX (rightArrowId, leftArrowId, aImages, sectionContainer) { //Assign a value to parameter counter on each container
  ...
  function PreviousNextClass () {
    //fResetContainersPosition ();
    var nextPos = 0;
    var ii      = 0;
    console.log ("ii: ", ii);
    /*this.ii                    = 0;
     console.log ("iia: ", this.ii);*/

    /*fXSlideContainer (horizonImagesContainerId, nextPos, leftArrowHorizon, rightArrowHorizon);
     fXSlideContainer (staxImagesContainerId, nextPos, leftArrowStax, rightArrowStax);
     fXSlideContainer (ownImagesContainerId, nextPos, leftArrowOwn, rightArrowOwn);*/

    this.fNextPreviousButtons = function fNextPreviousButtons (rightArrow, leftArrow, aryImages, sectionContainer) { //Assign a value to parameter counter on each container
      //var rightArrowElem = document.getElementById (rightArrow);
      ...
      this.RightArrow.click (function () {
        ...
        var getContainerWidth = me.SectionContainer.width();
        console.log ("getContainerWidth: ",getContainerWidth);
        var getContainerXpos = -nextPos;
        console.log ("getContainerXpos: ",getContainerXpos);
        console.log ("colRightWidth: ",colRightWidth);
        console.log ("ii: ",ii);

        imgXpos = -nextPos;
        multiplier = ii; //Clicked count to be multiplied by the recent image width
  ...
  function fSetTimeout () {
    console.log ("fSetTimeout:--------");
    //myVar = setTimeout(function(){ alert("Hello"); }, 3000);
    //myVar = setTimeout(fInitMediaQueries (), 3000);
    myVar = setTimeout (fMediaQueries (), 3000);
   ...
   *  ix: counter
   ****************************************************************/
  function fLoadContainerTicker () {
    tMx.ticker.addEventListener ("tick", fTimer);
    function fTimer () {
      /**-----===( Load Media Queries )===-----**/
      //fMediaQueries ();
      //fLoadNextPreviousButtons();
      newXpos = colRightWidth * multiplier;
      console.log("newXpos: ",newXpos);
      staxImagesContainerId.css ({"left": newXpos});
      ix++;
      if (ix >= 2) {
        fRemoveEvntListnr (fTimer);
      }
  ...
  function fPrevNextButtons () {
    ...
    prevNxtBtn1.fNextPreviousButtons (rightArrowHorizon, leftArrowHorizon, aHorizonImages, horizonImagesContainerId);
    prevNxtBtn2.fNextPreviousButtons (rightArrowStax, leftArrowStax, aStaxImages, staxImagesContainerId);
    prevNxtBtn3.fNextPreviousButtons (rightArrowOwn, leftArrowOwn, aOwnImages, ownImagesContainerId);
  }

  //fPrevNextButtons ();

  function fResetContainersPosition () {
    //console.log ("fResetContainersPosition: ", fResetContainersPosition);
    //fXSlider (horizonImagesContainerId, 0);
    //fXSlider (staxImagesContainerId, 0);
    //fXSlider (ownImagesContainerId, 0);
    for (var i = 0; i < aImgsContainer.length; i++) {
      fXSlider (aImgsContainer[i], 0);
      aRightArrows[i].show ();
      aLeftArrows[i].hide ();
  ...
  function fMediaQueries () {
    ...
    /**-----===( TO DO: When re-sizing window get the clicked amount mulitplied
     * by the current image width and used that number to re-position the container )===-----**/

    newXpos = colRightWidth * multiplier;
    console.log("newXpos: ",newXpos);

    //fRepositionContainer (staxImagesContainerId, newXpos);
    //staxImagesContainerId.css ({"left": newXpos});
    fXSlider (staxImagesContainerId, -newXpos); //Used the css left instead of the x matrix
    //staxImagesContainerId.style.left = newXpos;
    //tMx.to (staxImagesContainerId, animTym, {css: {x: -newXpos}, ease: easePower});

    //$("#staxImagesContainerId").parent().css({position: 'relative'});
    //$("#staxImagesContainerId").css({left: 200, position:'absolute'});

    //fLoadContainerTicker ();
  ...
  /**-------------===========( Next & Previous Click Functions )===========-------------**/
      ...
      //var rightArrowElem = document.getElementById ("rightArrowHorizon");
  var rightArrowElem = $ ("#rightArrowHorizon");
  var leftArrowElem  = $ ("#rightArrowHorizon");

  //var prevNxtBtn1 = new PreviousNextClass ();
  //prevNxtBtn1.fNextPreviousButtons (rightArrowElem, leftArrowElem, aHorizonImages, horizonImagesContainerId);

  //PreviousNextClass (rightArrowHorizon, leftArrowHorizon, aHorizonImages, horizonImagesContainerId);
  ...
  /**-----------=====| fReload Function |=====-----------**/
  /** Re-loads the whole page.
   *  This is what I found to be the easiest and safest way
   *  to re-load the fMediaQueries function.
   *  It should re-load the same vertical position of the page
   *  instead of resetting the position all the way up.
   *  This function is called when the page is re-sized.
   ****************************************************************/
  function fReload (reload) {
    console.log ("fReload: ");
    //location.reload();
    //window.location.reload();
    //window.location.href = "http://localhost:63342/gdSites/JavaScript_5/GDScrollSite2016/GDScroll_Index.html#anchorStax";
    window.location.hash = "#anchorStax";
    window.location.reload (true);
  }

  /*var currentWindowWidth = windowInnerWidth;
   function fReloadWindowOnce(){
   if(windowInnerWidth > currentWindowWidth || windowInnerWidth < currentWindowWidth){
  ...
  /!*Copyright © 2014 Louis-Rémi Babé*!/
  /!*MIT Licensed - http://louisremi.mit-license.org/!*!/
  function on_resize (c, t) {
    onresize = function () {
      console.log ("on_resize function:--------");
      clearTimeout (t);
      t = setTimeout (c, 100)
    };
    return c
  ...
  //window.addEventListener ('resize', fSetTimeout);
  //window.addEventListener ('resize', fReload);

  window.addEventListener ('resize', fInitMediaQueries);
  //window.onresize = function () {fReload};
